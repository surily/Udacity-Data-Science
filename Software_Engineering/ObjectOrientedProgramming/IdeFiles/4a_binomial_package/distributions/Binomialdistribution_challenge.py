...
class Binomial(Distribution):
    ...
    def __init__(self, prob = 0.25 , num_trails = 100):   
    ...
    # TODO: write a method plot_bar() that outputs a bar chart of the data set according to the following specifications.
        ...
        """
    def plot_bar(self):
        # plt.hist(x = self.data, bins = 'auto')
        # plt.title('Histogram')
        # plt.xlabel('Data')
    ...
    #TODO: Calculate the probability density function of the binomial distribution
        ...
        """
    def pdf(self, k):
        combination_num = math.factorial(self.n) / (math.factorial(k
            ) * math.factorial(self.n-k))
        pdf_pro = combination_num * ((self.p)** k) * (
                  (1-self.p) ** (self.n-k))
        return pdf_pro
